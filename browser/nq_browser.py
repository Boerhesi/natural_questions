...
python nq_browser.py --nq_jsonl=$NQ_DATA_DIR/train/nq-train-49.jsonl.gz
...
flags.DEFINE_integer('max_examples', 600,
...
class Example(object):
    ...
    def __init__(self, json_example):
        ...
        if self.has_short_answer:
            ...
            self.short_answers_texts = [
                ', '.join([
                    self.render_span(s['start_byte'], s['end_byte']).decode('utf-8')
    ...
    """
        ...
        if html_tag == '</Table>' and self.render_span(
                ...
                long_answer['end_byte']).count('<TR>') > 30:
...
def load_examples(fileobj):
  ...
  """
    ...
    examples = {}
    if FLAGS.gzipped:
        _load(examples, gzip.GzipFile(fileobj))
    else:
...
def main(unused_argv):
    # with open(FLAGS.nq_jsonl, encoding='utf-8', errors='ignore') as fileobj:
    #     examples = load_examples(fileobj)
    examples = load_examples(FLAGS.nq_jsonl)
